...
try {
  ...
  (
    window as typeof window & { getComputedStyle: GetComputedStyle }
  ).getComputedStyle = (node: Element, pseudoElt?: string | null) => {
    ...
    Object.entries(styleRules).forEach(([name, values]) => {
      collectedStyles[name as any] = Object.entries(values).reduce(
        ([prevSpecificy, prevValue], [specificy, value]) =>
          prevSpecificy > specificy
